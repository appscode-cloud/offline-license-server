...
PRODUCT_NAME       := offline-license-server
ENFORCE_LICENSE    ?=
...
install:
	...
	helm upgrade -i offline-license-server charts/offline-license-server --wait \
...
uninstall:
	@cd ../installer; \
	helm uninstall offline-license-server --namespace=$(KUBE_NAMESPACE) || true
